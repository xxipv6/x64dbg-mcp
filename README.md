...
| `dump_mem` | 将内存转储到调试器主机的 `.\\plugins\\mcp_dump\\` 目录 |
...
`dump_mem` 会忽略调用方传入路径中的目录部分，只提取文件名，并始终将文件写入调试器主机上的 `.\\plugins\\mcp_dump\\` 目录。

例如传入 `./plugins/mcp_dump/test_dump.bin`、`/tmp/test_dump.bin` 或 `test_dump.bin`，最终都会写成：

- `.\\plugins\\mcp_dump\\test_dump.bin`
