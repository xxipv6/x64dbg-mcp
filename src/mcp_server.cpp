...
ToolMap& GetRegisteredTools()
{
    static ToolMap tools = [] {
        ToolMap registered;
...
void LogToolCall(const std::string& toolName, const json& toolParams)
{
...
void LogToolResult(const std::string& toolName, const json& toolResult)
{
...
json NormalizeToolContent(const json& toolResult)
{
...
{
    ToolMap& tools = GetRegisteredTools();
    ...
    if (method == "tools/call")
    {
        ...
        try
        {
            ...
            return MakeResponse(id, {{"content", NormalizeToolContent(toolResult)}}).dump();
        }
